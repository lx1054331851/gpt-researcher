...
const Modal: React.FC<ChatBoxProps> = ({ chatBoxSettings, setChatBoxSettings, iconOnly = false }) => {
            ...
            <div className="relative flex flex-col rounded-lg overflow-hidden bg-gray-900 border border-gray-800/60 shadow-md hover:shadow-teal-400/10 transition-shadow duration-300">
              ...
              <div className="bg-gray-900 p-5 border-b border-gray-800">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold text-white">
                    ...
                    <span className="text-teal-400">{t("settings.preferencesTitle")}</span>
                  </h3>
                  <button
              ...
              <div className="relative p-6 flex-auto bg-gray-900/95 modal-content">
                {false && (<div className="tabs mb-4">
                  <button onClick={() => setActiveTab('report_settings')} className={`tab-button ${activeTab === 'report_settings' ? 'active' : ''}`}>{t("settings.reportSettingsTab")}</button>
                </div>)}
  ...
  return (
    <div className="settings">
      <button
        className={
          iconOnly
            ? "inline-flex h-9 w-9 items-center justify-center rounded-full border border-gray-600/60 bg-gray-900/70 text-gray-100 transition-colors hover:bg-gray-800"
            : "bg-gray-900 text-white px-6 py-3 rounded-lg shadow-sm hover:shadow-teal-400/10 transition-all duration-300 border border-gray-800 hover:border-teal-500/30"
        }
        type="button"
      ...
      >
        <span className="flex items-center">
          <svg xmlns="http://www.w3.org/2000/svg" className={iconOnly ? "h-5 w-5" : "h-5 w-5 mr-2"} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {!iconOnly && t("settings.preferencesTitle")}
        </span>
      </button>
